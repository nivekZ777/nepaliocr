...
void Separate::CheckMattra(ArrayList* lineStart,ArrayList* lineEnd,ArrayList** wordStart,ArrayList** wordEnd)
{
	...
	/// golmal Coding start 
	//System::Drawing::Bitmap * myTempBitmap = new System::Drawing::Bitmap(this->xSize,this->ySize);
	System::Drawing::Graphics * tempGraphicsCopier; 
	tempGraphicsCopier = Graphics::FromImage(myGlobalBitmap);
	...
	while(lineStartEnumerator->MoveNext() && lineEndEnumerator->MoveNext())
	{
		...
		for(int j=0;j<this->xSize;j++)
		{
			for(int k=0;k<cntMattra;k++)
			{
				...
				tempGraphicsCopier->DrawLine(p,0,mIndex[k],this->xSize,mIndex[k]);
	...
	tempGraphicsCopier->Dispose();

	System::String * myfilename;
//	System::Windows::Forms::MessageBox::Show(this->applicationPath);
	myfilename = myfilename->Concat(myfilename,this->applicationPath,"\\eurotext.tif");

	//myGlobalBitmap->Save(".\\eurotext.bmp");
	//System::Drawing::Bitmap *myTiffImage;
	//myTiffImage-> = System::Drawing::Imaging::ImageFormat::Tiff;
//	myTiffImage = System::Drawing::ImageFormatConverter(System::Drawing::Imaging::ImageFormat::Tiff);
	myGlobalBitmap->SetResolution(300,300);

	myGlobalBitmap->Save(myfilename,System::Drawing::Imaging::ImageFormat::Tiff);
	//myGlobalBitmap->Save(myfilename);
	myGlobalBitmap->Dispose();
}
...
void Separate::applicationPathResolver(System::String *mystring){
	this->applicationPath = mystring;
}
