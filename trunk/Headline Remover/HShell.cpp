...
static	Boolean  natReadOrder  = FALSE;  			   //< Preserve natural mach read order   
													   //	 >								  
static	Boolean  natWriteOrder	= FALSE;			   //< Preserve natural mach write		  
													   //	 order >						  
...
*/
Boolean  GetFileNameExt( char * logfn, char * actfn, 
						 long * st, long * en )
{
	int  		i, 
				 noccs;
	...
	noccs = 0;
	for( i = 0, p = extFiles; i < extFileUsed; 
		 i++, p++ )
	{
	...
	*en = p->enindex;
	if( trace & T_EXF )
		printf( "%sFile Ext found: %s=%s[%ld,%ld]\n", 
				( ambiguous )?"Ambiguous ":"", logfn, actfn, *st, *en );
	if( ambiguous )
		HError( -1, 
		"GetFileNameExt: ambiguous extended file name %s=%s[%ld,%ld]", 
		logfn, actfn, *st, *en );
...
static	ConfigEntry  * FindConfEntry( char * user, 
									  char * name )
{
	ConfigEntry  *	e;
	char		 *	s;
...
static	ReturnStatus  ReadConfigFile( char * fname )
{
	...
	if( recurse++ > 15 )
	{
		HRError( 5050, "ReadConfigFile: max #include depth reached (%s)", 
				 fname );
		recurse--;
		return ( FAIL );
	}

	if( InitSource( fname, &src, NoFilter )
		 < SUCCESS )
	{
...
int GetConfig( char * user, Boolean  incGlob, 
			   ConfParam ** list, int  max )
{
	ConfigEntry  *	e;
	int  			found  = 0;
		...
		for( e = confList; e != NULL; e = e->next )
		{
				 ...
				 && strcmp( e->param.user, user ) == 0 )
			{
				...
				if( list != NULL )
				{
					if( found > max )
						HError( 5071, 
						"GetConfig: more than %d for user %s%s", max, 
						user == NULL?"<null>":user, 
						incGlob?"with globals":"" );
					*list++ = &e->param;
				}
			}
		}
...
static	int FindConfParm( ConfParam ** list, int  size, 
						  char * name, ConfKind  kind )
{
	int i;
...
Boolean  HasConfParm( ConfParam ** list, int  size, 
					  char * name )
{
	bool	 var;
	Boolean  temp;
	var = FindConfParm( list, size, name, AnyCKind ) != -1;
	if( var == true )
		temp = TRUE;
	else
		temp = FALSE;
...
Boolean  GetConfStr( ConfParam ** list, int  size, 
					 char * name, char * str )
{
	int i;

	if( ( i = FindConfParm( list, size, name, 
							StrCKind ) )
		 != -1 )
	{
...
Boolean  GetConfBool( ConfParam ** list, int  size, 
					  char * name, Boolean * b )
{
	int i;
...
Boolean  GetConfInt( ConfParam ** list, int  size, 
					 char * name, int * ival )
{
	int i;
...
Boolean  GetConfFlt( ConfParam ** list, int  size, 
					 char * name, double * fval )
{
	int i;
	...
	{
		*fval
		= ( list[ i ]->kind == FltCKind )?list[ i ]->val.f:list[ i ]->val.i;
		return TRUE;
	}
	return FALSE;
}
...
static	char * ScriptWord( void )
{
	...
	if( ch == '\'' || ch == '"' )
	{
		...
		while( ch != qch && ch != EOF )
		{
			scriptBuf[ i++ ] = ch;					   //	   #### ge: should check for	  
													   //		   overflow of scriptBuf	  
			ch				 = fgetc( script );
		}
		if( ch == EOF )
...
static	void ArgError( char * s )
{
	if( NextArg( ) == NOARG )
		...
		HError( 5021, "ArgError: %s arg requested but next is %s", s, 
				GetNextArg( FALSE ) );
}
...
int GetChkedInt( int  min, int	max, char * swtname )
{
	...
	if( NextArg( ) != INTARG )
		HError( 5021, "GetChkedInt: Integer Arg Required for %s option", 
				swtname );
	val = GetIntArg( );
	if( val < min || val > max )
		HError( 5022, "GetChkedInt: Integer arg out of range in %s option", 
				swtname );
	return val;
}

/* EXPORT->GetChkedLong: range checked version of GetIntArg */
long  GetChkedLong( long  min, long  max, 
					char * swtname )
{
	long  val;

	if( NextArg( ) != INTARG )
		HError( 5021, "GetChkedLong: Integer Arg Required for %s option", 
				swtname );
	val = GetLongArg( );
	if( val < min || val > max )
		HError( 5022, "GetChkedLong: Long arg out of range in %s option", 
				swtname );
	return val;
}

/* EXPORT->GetChkedFlt: range checked version of GetFltArg */
float GetChkedFlt( float  min, float  max, 
				   char * swtname )
{
	float val;
	...
	if( NextArg( ) != INTARG
		 && NextArg( ) != FLOATARG )
		HError( 5021, "GetChkedFlt: Float Arg Required for %s option", 
				swtname );
	val = GetFltArg( );
	if( val < min || val > max )
		HError( 5022, "GetChkedFlt: Float arg out of range in %s option", 
				swtname );
	return val;
}
...
static	Boolean  FilterSet( IOFilter  filter, char * s )
{
	...
	if( ( env = getenv( filtermap[ filter ] ) )
		 != NULL )
	{
		strcpy( s, env );
		return TRUE;
	}
	else  if( GetConfStr( cParm, nParm, 
						filtermap[ filter ], s ) )
		return TRUE;
	else
		return FALSE;
...
FILE  * FOpen( char * fname, IOFilter  filter, 
			   Boolean * isPipe )
{
	FILE	 *	f;
	int  		i;
...
ReturnStatus  InitSource( char * fname, Source * src, 
						  IOFilter	filter )
{
	CheckFn( fname );
	strcpy( src->name, fname );
	if( ( src->f
		  = FOpen( fname, filter, &( src->isPipe ) ) )
		 == NULL )
	{
...
char * SrcPosition( Source	src, char * s )
{
	...
	else
	{
		...
		sprintf( s, "line %d/col %d/char %ld in %s", line, col, pos, 
				 src.name );
	}
	return s;
}
...
void ReadUntilLine( Source * src, char * s )
{
	...
	do
	{
		if( !ReadLine( src, buf ) )
			HError( 5013, 
					"ReadUntilLine: reached EOF while scanning for '%s'", s );
...
Boolean  ReadStringWithLen( Source * src, char * s, 
							int  buflen )
{
	int i, 
		 c, 
...
void WriteString( FILE * f, char * s, char	q )
{
	...
	for( p = ( unsigned char * )s; *p; p++ )
	{
		...
		else  if( isprint( *p ) || noNumEscapes )
			fputc( *p, f );
		else
		{
			n = *p;
...
static	Boolean  IsVAXOrder( void )
{
	...
	*( pc + 1 ) = 0;								   //  store bytes 1 0					  
	var  		= ( x == 1 );						   //  does it read back as 1?			  
	if( var == true )
		temp = TRUE;
	else
		temp = FALSE;
...
Boolean  RawReadShort( Source * src, short * s, int  n, 
					   Boolean	bin, Boolean  swap )
{
		   int	  j, 
				   k, 
				   count  = 0, 
	...
	if( bin )
	{
		if( fread( s, sizeof( short ), n, src->f )
			 != n )
			return FALSE;
		if( swap )
			for( p = s, j = 0; j < n; p++, j++ )
				SwapShort( p );  					   //		 Need to swap to machine	  
													   //			 order					  
...
Boolean  RawReadInt( Source * src, int * i, int  n, 
					 Boolean  bin, Boolean	swap )
{
	int    j, 
			k, 
	...
	if( bin )
	{
		if( fread( i, sizeof( int ), n, src->f )
			 != n )
			return FALSE;
		if( swap )
			for( p = i, j = 0; j < n; p++, j++ )
				SwapInt32( ( int32 * )p );			   //		 Read in SUNSO unless		  
													   //			 natReadOrder=T  		  
...
Boolean  RawReadFloat( Source * src, float * x, int  n, 
					   Boolean	bin, Boolean  swap )
{
	int  	 k, 
			  count  = 0, 
	...
	if( bin )
	{
		if( fread( x, sizeof( float ), n, src->f )
			 != n )
			return FALSE;
		if( swap )
			for( p = x, j = 0; j < n; p++, j++ )
...
/* EXPORT->ReadShort: read n short's from src in ascii or binary */
Boolean  ReadShort( Source * src, short * s, int  n, 
					Boolean  binary )
{
	bool	 var  = ( vaxOrder && !natReadOrder );
	Boolean  temp;
	if( var == true )
		temp = TRUE;
	else
	...
	temp = RawReadShort( src, s, n, binary, temp );
	return temp;
}

/* EXPORT->ReadInt: read n ints from src in ascii or binary */
Boolean  ReadInt( Source * src, int * i, int  n, 
				  Boolean  binary )
{
	bool	 var  = ( vaxOrder && !natReadOrder );
	Boolean  temp;
	...
	return ( RawReadInt( src, i, n, binary, temp ) );
}

/* EXPORT->ReadFloat: read n floats from src in ascii or binary */
Boolean  ReadFloat( Source * src, float * x, int  n, 
					Boolean  binary )
{
	bool	 var  = ( vaxOrder && !natReadOrder );
	Boolean  temp;
	...
	return ( RawReadFloat( src, x, n, binary, temp ) );
}
...
void WriteShort( FILE * f, short * s, int  n, 
				 Boolean  binary )
{
		   int	  j, 
				   x;
	...
	if( binary )
	{
		...
		{
			for( p = s, j = 0; j < n; p++, j++ )
				SwapShort( p );  					   //		 Write in SUNSO unless		  
													   //			 natWriteOrder=T		  
		}
...
void WriteInt( FILE * f, int * i, int  n, 
			   Boolean	binary )
{
	int    j;
	int *  p;
	...
	if( binary )
	{
		...
		{
			for( p = i, j = 0; j < n; p++, j++ )
				SwapInt32( ( int32 * )p );			   //		 Write in SUNSO unless		  
													   //			 natWriteOrder=T		  
		}
...
char *	MakeFN( char * fn, char * path, char * ext, 
				char * s )
{
	char newPath[ 255 ],							   //  < components of new fn >  		  
		  base[ 64 ], 
		  newExt[ 20 ];
	int  i;
...
char *	CounterFN( char * prefix, char * suffix, 
				   int	count, int	width, char * s )
{
	char num[ 20 ];
...
Boolean  RMatch( char * s, char * p, int  slen, 
				 int  minplen, int	numstars )
{
	if( slen == 0 && minplen == 0 )
		return TRUE;
	...
	{
		bool var;
		var = RMatch( s + 1, p + 1, slen - 1, minplen, numstars - 1 )
...
ReturnStatus  InitShell( int  argc, char * argv[ ], 
						 char * ver, char * sccs )
{
	char	 *	fn;
	int  		i, 
   ...
   _fmode = _O_BINARY;
	...
	for( i = 1; i < argc; i++ )
	{
		...
		else  if( strcmp( argv[ i ], "-C" ) == 0 )
		{
			...
			if( ReadConfigFile( argv[ i ] )
				 < SUCCESS )
			{
				HRError( 5020, "InitShell: ReadConfigFile failed on file %s", 
						 argv[ i ] );
				return ( FAIL );
			}
		}
		else  if( strcmp( argv[ i ], "-S" ) == 0 )
		{
			...
			if( SetScriptFile( argv[ i ] )
				 < SUCCESS )
			{
				HRError( 5020, "InitShell: SetScriptFile failed on file %s", 
						 argv[ i ] );
				return ( FAIL );
			}
		}
	...
	if( nParm > 0 )
	{
		if( GetConfInt( cParm, nParm, "TRACE", &i ) )
			trace = i;
		if( GetConfBool( cParm, nParm, 
						 "NONUMESCAPES", &b ) )
			noNumEscapes = b;
		if( GetConfBool( cParm, nParm, "ABORTONERR", 
						 &b ) )
			abortOnError = b;
		if( GetConfBool( cParm, nParm, 
						 "NATURALREADORDER", &b ) )
		...
		if( GetConfBool( cParm, nParm, 
						 "EXTENDFILENAMES", &b ) )
			extendedFileNames = b;
		if( GetConfInt( cParm, nParm, "MAXTRYOPEN", 
						&i ) )
		{
			maxTry = i;
			if( maxTry < 1 || maxTry > 25 )
			{
				HRError( 5073, "InitShell: MAXTRYOPEN out of range (%d)", 
						 maxTry );
				maxTry = 1;
			}
		}
...
char * GiveTime( TimeStruct * t )
{
	...
	clock_t  		 clock_time;
	int  			 tenth_millisecs, 
					  clock_tenth_millisecs;
	if( gettimeofday( &time, NULL ) )
		HError( 1, "Error getting time of day." );
	clock_time = clock( );
	tenth_millisecs
	= ( time.tv_sec - t->time.tv_sec ) * 10000
	   + ( time.tv_usec - t->time.tv_usec ) / 100;
	clock_tenth_millisecs = ( clock_time - t->clock_time ) / 100;

	sprintf( t->timestr, "%d.%04d/clock %d.%04d", tenth_millisecs / 10000, 
	tenth_millisecs % 10000, clock_tenth_millisecs / 10000, 
	clock_tenth_millisecs % 10000 );
	return t->timestr;
}
#endif
