/* The original code was forked from : http://code.google.com/p/nepaliocr/source/browse/trunk/removeNoise.h?spec=svn146&r=138
 * 
 */
...
namespace NOCR
{
    ...
    public class removeNoise
    {
        ...
        public removeNoise(System.Drawing.Bitmap BinaryImage, int[,] ImgArray)
        {
        ...
        internal System.Drawing.Bitmap GetClearImage()
        {
            while (true)
            {
                numFill = 0;
                for (int i = stRwIndx; i < edRwIndx; i++)//every row of picture starting from row 2
                {
                    for (int j = stClIndx; j < edClIndx; j++)//every column of the row starting from column 2
                    {
                        ...
                        {
                            for (int rowOfImage = (i - 1), rowOfWindow = 0; rowOfImage <= (i + 1) & rowOfWindow <= 2; rowOfImage++, rowOfWindow++)
                            {//to extract window 
                                for (int colOfImage = (j - 1), colOfWindow = 0; colOfImage <= (j + 1) & colOfWindow <= 2; colOfImage++, colOfWindow++)
                                {//to extract window 
                            ...
                            if (pass == 1)
                            {
                                ...
                                this.safaImage.SetPixel(j, i, System.Drawing.Color.White);
                                numFill = 1;
                            }
